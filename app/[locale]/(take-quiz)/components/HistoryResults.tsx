...
const HistoryResults = () => {
  ...
  return (
    <Container>
      <section className="bg-default-100 dark:bg-gray-800 py-8 max-w-3xl mx-auto px-6 flex flex-col gap-6 mt-8 rounded-lg">
        <Progress
        ...
        <div className="flex justify-between">
          <h3 className="text-4xl text-foreground-700 dark:text-white font-semibold">
            {t("quizResults")}
          </h3>
          <p className="text-foreground-600 dark:text-gray-300">
        ...
        {quizDetails.questions.map((question, questionIndex) => {
          ...
          return (
            <div
              key={question.id}
              className="bg-default-100 dark:bg-gray-700 p-4 mb-4 border-dashed border-2 dark:border-gray-600 rounded-lg flex justify-between items-start shadow-sm"
            >
                  ...
                  {question.answers.map((answer, answerIndex) => {
                    ...
                    return (
                      <div
                        key={answer.id}
                        className={cn(
                          "flex items-center p-2 rounded-lg cursor-pointer w-full bg-white dark:bg-gray-600 justify-between",
                          answer.isCorrect && "bg-success-100 dark:bg-green-700",
                          isSelected && !answer.isCorrect && "bg-danger-100 dark:bg-red-700"
                        )}
                      >
                        <div className="flex">
